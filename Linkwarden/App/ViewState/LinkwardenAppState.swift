...
class LinkwardenAppState: ObservableObject {
    ...
    private func validateSession() async -> Bool {
        ...
        case .success(let session):
            ...
            // TODO: ZVZV Check if the session expiry is the same as the one stored in coredata if so do nothing, if not update the expiry date.
        ...
        case .failure(let error):
            LLogger.shared.critical("Session is invalid \(error)")
            return false
        }
    }
